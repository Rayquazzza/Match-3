...
public static class GameServiceLocator
{
                                  ...
                                  bool isOptional = false, string debugName = null) where T : class
    {
        if (service == null)
        {
            Debug.LogError($"[GameServiceLocator] Cannot register null service of type {typeof(T).Name}");
            return;
        }
        ...
        if (services.ContainsKey(serviceType))
        {
            ...
            {
                Debug.LogWarning($"[GameServiceLocator] Overriding existing {existing.lifecycle} service: {existing.debugName}");
            }
        }
        ...
        Debug.Log($"[GameServiceLocator] Registered {lifecycle} service: {entry.debugName}");
    }
    ...
    public static T Get<T>() where T : class
    {
        ...
        {
            Debug.Log($"[GameServiceLocator] Optional service {serviceType.Name} not found, returning null");
            return null;
        }

        var errorMsg = $"[GameServiceLocator] Required service {serviceType.Name} not registered!\n" +
    ...
    public static void Replace<T>(T newService, string debugName = "Temporary") where T : class
    {
        if (newService == null)
        {
            Debug.LogError($"[GameServiceLocator] Cannot replace with null service of type {typeof(T).Name}");
            return;
        }
    ...
    public static void CleanupSceneScopedServices()
    {
        ...
        foreach (var kvp in services)
        {
            if (kvp.Value.lifecycle == ServiceLifecycle.SceneScoped)
            {
                ...
                Debug.Log($"[GameServiceLocator] Cleaning up scene-scoped service: {kvp.Value.debugName}");
            }
        }
    ...
    private static void Unregister(Type serviceType)
    {
        if (services.Remove(serviceType))
        {
            ...
            Debug.Log($"[GameServiceLocator] Unregistered service: {serviceType.Name}");
        }
    }
